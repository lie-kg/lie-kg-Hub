...
setup_ubuntu_debian() {
    ...
    print_status "Step 2: Installing base packages..."
    ...
    # Step 3: Add PHP repo (Ubuntu only)
    if [[ "$OS" == "ubuntu" ]]; then
        print_status "Step 3: Adding PHP repository (Ubuntu)..."
        ...
        print_status "Step 3: Skipped (Debian uses default repositories)"
    fi

    # Step 4: Add universe repo (Ubuntu only)
    if [[ "$OS" == "ubuntu" ]]; then
        print_status "Step 4: Adding universe repository..."
        ...
        print_status "Step 4: Skipped (Not needed for Debian)"
    fi

    # Step 5: Install dependencies
    print_status "Step 5: Installing PHP and other dependencies..."

    if [[ "$OS" == "ubuntu" ]]; then
        apt -y install php8.2 php8.2-{common,cli,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip}
    else
        # Debian safe install (no hard version)
        apt -y install php php-cli php-gd php-mysql php-mbstring php-bcmath php-xml php-fpm php-curl php-zip
    fi

    # Common packages
    apt -y install mariadb-server nginx tar unzip zip git redis-server dos2unix
